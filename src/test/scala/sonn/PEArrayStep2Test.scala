...
class PEArrayStep2Test(c: PEArrayTestTop) extends PeekPokeTester(c) {
...
class PEArrayStep2Tester extends ChiselFlatSpec {
  "running with --generate-vcd-output on" should "create a vcd file from your test" in {
    iotesters.Driver.execute(
      Array(
        ...
        "on",
        "--target-dir",
        "test_run_dir/PEArrayStep2",
        "--top-name",
        "make_Test_vcd",
        "--backend-name",
        "verilator"
      ),
      () => new PEArrayTestTop((3, 24), 16)
    ) { c =>
      new PEArrayStep2Test(c)
    } should be(true)
    new File("test_run_dir/PEArrayStep2/PEArrayTestTop.vcd").exists should be(
      true
    )
  }
}
