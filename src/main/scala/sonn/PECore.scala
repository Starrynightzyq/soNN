...
class PECore(w:Int = 16)
  extends Module{
    ...
    //   val regConfig = Flipped(DecoupledIO(new PEConfigReg(16)))
      ...
      val pSumOut = DecoupledIO(SInt((4*w).W))

      val stateOut = Output(UInt(4.W))
      val clrDoneOut = Output(UInt(1.W))
    })

    val configReg = WireInit(io.regConfig)
    ...
    switch(state) {
        ...
        is(add) {
            ochannelCnt.value := 0.U
            io.pSumIn.ready := 1.U
            when(io.pSumIn.fire() && io.pSumOut.ready === 1.U){
